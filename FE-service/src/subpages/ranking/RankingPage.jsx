...
const RankingPage = () => {
    ...
    if (isLoadingList) {
        return <div className="text-center p-10 text-xl text-pokemon-blue dark:text-pokemon-blue-light">Ładowanie rankingu...</div>;
    }
    if (isErrorList) {
    ...
    if (!combinedPokemonData || combinedPokemonData.length === 0) {
        return (
            <div className="p-4 h-full flex flex-col">
                <h1 className="text-3xl font-bold mb-6 text-center text-pokemon-blue-dark dark:text-pokemon-blue-light flex-shrink-0">Ranking Pokémon</h1>
                <div className="flex-grow flex items-center justify-center">
                    <p className="text-center text-pokemon-gray-dark dark:text-pokemon-gray-light">Brak danych Pokémonów do wyświetlenia.</p>
                </div>
            </div>
        );
    }
            ...
            <div className="mb-4 md:hidden flex-shrink-0">
                <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                    <div className="flex items-center gap-2">
                        <label htmlFor="mobile-stat-select" className="text-sm font-medium text-gray-700 dark:text-gray-300">Pokaż i sortuj wg:</label>
                        <select
                ...
                <div className="h-full overflow-y-auto relative">
                    <table className="min-w-full border-collapse">
                        ...
                        <tr>
                            ...
                            <th scope="col" className="px-2 md:px-3 py-3 text-center text-xs font-medium text-pokemon-gray-dark dark:text-gray-300 uppercase tracking-wider w-10 md:w-12">Rank</th>
                            {/* Image Header (Empty) */}
                            <th scope="col" className="px-2 md:px-3 py-3 text-center text-xs font-medium text-pokemon-gray-dark dark:text-gray-300 uppercase tracking-wider w-12 md:w-16"></th>
                            ...
                            <th scope="col" className="px-2 md:px-3 py-3 text-center text-xs font-medium text-pokemon-gray-dark dark:text-gray-300 uppercase tracking-wider">Typy</th>

                            {/* Desktop Stat Headers (Hidden on Mobile) */}
                            <SortableHeaderCell columnKey="wins" sortBy={sortBy} sortOrder={sortOrder} onSort={handleSort} className="hidden md:table-cell">Wygrane</SortableHeaderCell>
                            <SortableHeaderCell columnKey="base_experience" sortBy={sortBy} sortOrder={sortOrder} onSort={handleSort} className="hidden md:table-cell">Dośw.</SortableHeaderCell>
                            <SortableHeaderCell columnKey="weight" sortBy={sortBy} sortOrder={sortOrder} onSort={handleSort} className="hidden md:table-cell">Waga</SortableHeaderCell>
                            <SortableHeaderCell columnKey="height" sortBy={sortBy} sortOrder={sortOrder} onSort={handleSort} className="hidden md:table-cell">Wzrost</SortableHeaderCell>
                        ...
                        {sortedRankedPokemons.map((pokemon, index) => (
                            <tr key={pokemon.id} className="hover:bg-pokemon-gray-light/30 dark:hover:bg-pokemon-gray-darker/50 transition-colors">
                                ...
                                {/* Name Cell */}
                                <td className={clsx("px-2 md:px-3 py-3 whitespace-nowrap text-sm align-middle text-left", sortBy === 'name' ? 'font-bold text-pokemon-blue dark:text-pokemon-yellow' : 'font-semibold text-pokemon-gray-darker dark:text-gray-200')}>{capitalizeWords(pokemon.name)}</td>
                                {/* Types Cell */}
                                ...
                                <td className={clsx("hidden md:table-cell px-3 py-3 whitespace-nowrap text-sm text-center align-middle", sortBy === 'wins' ? 'font-bold text-pokemon-blue dark:text-pokemon-yellow' : 'text-pokemon-gray-darker dark:text-gray-300')}>{formatStatValue(pokemon, 'wins')}</td>
                                <td className={clsx("hidden md:table-cell px-3 py-3 whitespace-nowrap text-sm text-center align-middle", sortBy === 'base_experience' ? 'font-bold text-pokemon-blue dark:text-pokemon-yellow' : 'text-pokemon-gray-darker dark:text-gray-300')}>{formatStatValue(pokemon, 'base_experience')}</td>
                                <td className={clsx("hidden md:table-cell px-3 py-3 whitespace-nowrap text-sm text-center align-middle", sortBy === 'weight' ? 'font-bold text-pokemon-blue dark:text-pokemon-yellow' : 'text-pokemon-gray-darker dark:text-gray-300')}>{formatStatValue(pokemon, 'weight')}</td>
                                <td className={clsx("hidden md:table-cell px-3 py-3 whitespace-nowrap text-sm text-center align-middle", sortBy === 'height' ? 'font-bold text-pokemon-blue dark:text-pokemon-yellow' : 'text-pokemon-gray-darker dark:text-gray-300')}>{formatStatValue(pokemon, 'height')}</td>

                                {/* Mobile Stat Cell (Hidden on Desktop) */}
                                <td className={clsx("md:hidden table-cell px-2 py-3 whitespace-nowrap text-sm text-center align-middle", sortBy === displayedStat ? 'font-bold text-pokemon-blue dark:text-pokemon-yellow' : 'text-pokemon-gray-darker dark:text-gray-300')}>
