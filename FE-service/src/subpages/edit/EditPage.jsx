...
import { updateUserPokemonStats, getPokemonImageUrl } from '../../services/api/pokemon';
...
const EditPage = () => {
    ...
    const mutation = useMutation({
        mutationFn: ({ pokemonId, newData }) => updateUserPokemonStats(currentUser.id, pokemonId, newData),
        onSuccess: (updatedUserData, variables) => {
            ...
            const isCreating = variables.newData.isCustom === true && !currentUser?.pokemonStats?.[variables.pokemonId];
            // Get the name for the message (from newData if creating, or existing data if editing)
            const rawPokemonName = variables.newData.name ||
                selectedPokemonForEdit?.name || // Use name from selected pokemon if editing
                sortedPokemonList?.find(p => String(p.id) === String(variables.pokemonId))?.name ||
                `Pokemon #${variables.pokemonId}`;
            const pokemonName = capitalizeWords(rawPokemonName);

            const message = isCreating ? `Nowy pokemon ${pokemonName} został dodany!` : `Zmieniono atrybuty dla ${pokemonName}!`;
    ...
    if (isLoadingList) return <div className="p-4 text-center text-xl text-pokemon-blue dark:text-pokemon-blue-light">Ładowanie listy Pokémonów...</div>;
    if (isErrorList) return <div className="p-4 text-center text-xl text-pokemon-red dark:text-red-400">Błąd ładowania listy: {errorList?.message}</div>;
    if (!currentUser) return <div className="p-4 text-center">Musisz być zalogowany, aby edytować Pokémony.</div>;
    ...
    const handleSaveEdit = (pokemonId, formData) => {
        const originalPokemonData = sortedPokemonList?.find(p => String(p.id) === String(pokemonId));
        const existingStats = currentUser?.pokemonStats?.[String(pokemonId)] || {};

        const isApiPokemon = !existingStats.isCustom && pokemonId <= POKEMON_API_LIMIT;

        // Get original numeric values, defaulting to safe values
        const originalExperience = Number(existingStats.modified_base_experience ?? existingStats.base_experience ?? originalPokemonData?.base_experience) || 1; // Min 1
        const originalHeight = Number(existingStats.height ?? originalPokemonData?.height) || 0.1; // Min 0.1
        const originalWeight = Number(existingStats.weight ?? originalPokemonData?.weight) || 0.1; // Min 0.1
        ...
        if (Object.keys(newData).length > 0) {
            ...
            if (existingStats.wins !== undefined && newData.wins === undefined) newData.wins = existingStats.wins;
            if (existingStats.losses !== undefined && newData.losses === undefined) newData.losses = existingStats.losses;
            ...
            <div className={clsx(
                ...
                <div className="h-full overflow-y-auto relative">
                    <table className="min-w-full border-collapse">
                        ...
                        <tr>
                            <th scope="col" className="px-3 py-3 text-right text-xs font-medium text-pokemon-gray-dark dark:text-gray-300 uppercase tracking-wider w-10">#</th>
                            <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-pokemon-gray-dark dark:text-gray-300 uppercase tracking-wider">Pokémon</th>
                            <th scope="col" className="px-4 py-3 text-center text-xs font-medium text-pokemon-gray-dark dark:text-gray-300 uppercase tracking-wider w-24">Akcje</th>
                        </tr>
                        </thead>
                        ...
                        {sortedPokemonList.map((pokemon, index) => (
                            <tr key={pokemon.id} className="hover:bg-pokemon-gray-light/30 dark:hover:bg-pokemon-gray-darker/50 transition-colors duration-150 ease-in-out">
                                {/* Wyświetlanie indexu + 1 jako numeracji */}
                                <td className="px-3 py-2 whitespace-nowrap text-sm font-medium text-pokemon-gray-dark dark:text-gray-400 text-right align-middle">{index + 1}</td>
