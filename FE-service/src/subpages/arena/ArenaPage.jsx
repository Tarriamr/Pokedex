...
const FightStatusMessage = ({ displayFightingState, fightResultMessage }) => {
    let message = null;
    let messageClass = "text-center text-2xl font-bold text-white dark:text-gray-100 text-shadow-md my-4 px-4 py-2 bg-black bg-opacity-60 dark:bg-opacity-75 rounded-lg h-16 flex items-center justify-center"; // Added fixed height and flex centering
    ...
    return message ? <div className={messageClass}>{message}</div> : <div className="h-16"></div>; // Render placeholder div with same height
};
...
const ArenaPage = () => {
    ...
    const removePokemonMutation = useMutation({
        mutationFn: (pokemonIdToRemove) => {
            if (!currentUser || !pokemonIdToRemove) throw new Error("Brak ID użytkownika lub Pokemona.");
            const currentArenaIds = queryClient.getQueryData(['user', currentUser.id])?.arenaPokemonIds?.map(String) || [];
            const updatedArena = currentArenaIds.filter(id => id !== String(pokemonIdToRemove));
    ...
    const handleLeaveArena = useCallback(() => {
        if (leaveArenaMutation.isPending || isFightInProgress || isMutatingStats) return;
        leaveArenaMutation.mutate();
    }, [leaveArenaMutation, isFightInProgress, isMutatingStats]);

    const handleRemovePokemon = useCallback((pokemonIdToRemove) => {
        if (!pokemonIdToRemove || removePokemonMutation.isPending || isFightInProgress || isMutatingStats) return;
        removePokemonMutation.mutate(pokemonIdToRemove);
    }, [removePokemonMutation, isFightInProgress, isMutatingStats]);

    // --- Effects --- //
    useEffect(() => {
        if (!isMutatingStats && fightResult && isFightInProgress && !showResultDelayed) {
            clearTimeout(fightTimerRef.current);
    ...
    const canFight = useMemo(() =>
            arenaPokemonIds.length === 2 && !!pokemon1 && !!pokemon2 &&
            !isFightInProgress && !isMutatingStats && !hasLoadingError && !isLoadingData
        , [arenaPokemonIds.length, pokemon1, pokemon2, isFightInProgress, isMutatingStats, hasLoadingError, isLoadingData]);
    ...
    const fightResultMessage = useMemo(() => {
        ...
        if (fightResult.winner && fightResult.loser) {
            const winnerData = String(pokemon1?.id) === String(fightResult.winner.id) ? pokemon1 : pokemon2;
            const loserData = String(pokemon1?.id) === String(fightResult.loser.id) ? pokemon1 : pokemon2;
    ...
    const dataForSlot1 = displayFightingState ? pokemon1PreFightDataRef.current : pokemon1;
    const dataForSlot2 = displayFightingState ? pokemon2PreFightDataRef.current : pokemon2;
        ...
        <ArenaBackground>
            ...
            {!isLoadingData && !hasLoadingError && (
                <div className="w-full max-w-5xl mx-auto flex flex-col items-center transition-opacity duration-500 ease-in-out opacity-100">
                    <h1 className="text-4xl md:text-5xl font-bold text-white dark:text-gray-100 text-shadow-lg mb-8 md:mb-12 mt-4">Arena Pokemon</h1>
