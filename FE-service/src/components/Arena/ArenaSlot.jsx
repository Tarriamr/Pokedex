...
const ArenaSlot = ({
    ...
    const { id, name, base_experience, weight, wins, losses, types } = combinedPokemonData;
    ...
    if (showResultDelayed && fightResult) {
        const isWinner = !fightResult.draw && String(fightResult.winner?.id) === String(id);
        const isLoser = !fightResult.draw && String(fightResult.loser?.id) === String(id);
        if (isWinner) {
    ...
    return (
        ...
        >
            ...
            <img
                ...
                onError={(e) => { e.target.onerror = null; e.target.src = pokeballFallback; }}
            />

            {/* Pokemon Name */}
            <h3 className="text-xl font-bold capitalize text-white dark:text-gray-100 text-shadow-sm mb-2 text-center break-words">{capitalizeWords(name)}</h3>
