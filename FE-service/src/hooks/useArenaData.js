...
export const useArenaData = () => {
  ...
  const arenaPokemonIds = useMemo(
    () => userData?.arenaPokemonIds?.map(String) || [],
    [userData],
  );
  ...
  const pokemon1 = useMemo(() => {
    ...
    return combinePokemonData(
      basePokemon1Details,
      pokemonStats[pokemonId1],
      pokemonId1,
    );
  ...
  const isLoading =
    isLoadingUser ||
    (!!pokemonId1 && isLoadingDetails) ||
    (!!pokemonId2 && isLoadingDetails);

  const isError =
    isUserError ||
    (!!pokemonId1 && isErrorDetails) ||
    (!!pokemonId2 && isErrorDetails);
