...
const usePokemonList = (limit = POKEMON_API_LIMIT, userStats = null) => {
  const {
    data: basePokemonList,
    isLoading: isLoadingApiList,
    isError: isErrorApiList,
    error: errorApiList,
  } = useQuery({
  ...
  const customPokemonIds = useMemo(() => {
    return Object.keys(stats).filter((idStr) => {
  ...
  const isLoadingCustomDetails = customPokemonDetailsQueries.some(
    (q) => q.isLoading,
  );
  const isErrorCustomDetails = customPokemonDetailsQueries.some(
    (q) => q.isError,
  );
  const errorCustomDetails = customPokemonDetailsQueries.find(
    (q) => q.error,
  )?.error;
  ...
  const combinedPokemonList = useMemo(() => {
    ...
    const customDetailsMap = customPokemonDetailsQueries.reduce(
      (acc, queryResult) => {
        if (queryResult.isSuccess && queryResult.data) {
          acc[String(queryResult.data.id)] = queryResult.data;
        } else if (queryResult.isError) {
          console.warn(
            `Failed to fetch details for potential custom Pokemon ID: ${queryResult.queryKey[1]}`,
            queryResult.error,
          );
        }
        return acc;
    ...
    if (basePokemonList) {
      basePokemonList.forEach((basePokemon) => {
        ...
        if (!basePokemon || typeof basePokemon.id === "undefined") {
          console.warn(
            "[usePokemonList] Skipping invalid base Pokemon data entry:",
            basePokemon,
          );
          return;
        }
        ...
        const combined = combinePokemonData(
          basePokemon,
          userStat,
          pokemonIdStr,
        );
