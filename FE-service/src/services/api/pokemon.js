...
const _processPokemonData = (data) => {
    if (!data) return null;
    const { id, name, types, stats, weight, height, base_experience, abilities } = data;
        ...
        const processedList = detailsResponses
            .map((response, index) => {
                if (response.status === 'fulfilled') {
                    ...
                    console.error(`Error fetching details for ${results[index]?.name}:`, response.reason);
                    return null;
                }
            })
...
export const updateUserPokemonStats = async (userId, pokemonId, statsToUpdate) => {
    ...
    if (Object.keys(cleanStatsToUpdate).length === 0) {
        try {
            ...
            console.error(`[updateUserPokemonStats] Error fetching user data after skipping update for Pokemon ${pokemonIdStr}:`, fetchError);
            throw new Error('Nie udało się pobrać danych użytkownika po pominięciu aktualizacji.');
        }
    }
    ...
    try {
        ...
        const mergedStatsForPokemon = { ...existingStatsForPokemon, ...cleanStatsToUpdate };
        ...
        console.error(`Error updating stats for Pokemon ${pokemonId} (User ${userId}) via PATCH ${`/users/${userId}`}:`, error);
        const errorMessage = error.response?.data?.message || `Nie udało się zaktualizować statystyk dla Pokémona ${pokemonId}.`;
...
export const updateUserArena = async (userId, arenaPokemonIds) => {
    ...
    if (!Array.isArray(arenaPokemonIds)) throw new Error("arenaPokemonIds musi być tablicą.");

    const stringArenaPokemonIds = arenaPokemonIds.map(id => String(id));

    if (stringArenaPokemonIds.length > MAX_ARENA_POKEMONS) {
        throw new Error(`Arena może pomieścić maksymalnie ${MAX_ARENA_POKEMONS} Pokemony.`);
    }
