...
export const ThemeProvider = ({ children }) => {
  ...
  const updateThemeMutation = useMutation({
    mutationFn: ({ userId, newTheme }) =>
      updateUserPreferences(userId, { theme: newTheme }),
    ...
    if (isLoggedIn && currentUser?.preferences?.theme) {
      ...
      if (localStorage.getItem("theme") !== userTheme) {
        try {
          ...
          console.error(
            "[ThemeContext] Failed to remove theme from localStorage:",
            error,
          );
        }
      }
  ...
  useEffect(() => {
    ...
    if (!isLoggedIn) {
      try {
        ...
        console.error(
          "[ThemeContext] Failed to save theme to localStorage:",
          error,
        );
      }
    }
